import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css                  */import{p as l,g as d,t as u}from"./load-pet-cards-BZfxWR33.js";import"./footer-loader-Bpclrq5y.js";function h(){return new URLSearchParams(window.location.search).get("petId")}function m(t){return l.find(e=>e.id===t)}function g(){const t=h();if(!t){console.error("No pet ID provided");return}const e=m(t);if(!e){console.error("Pet not found");return}p(e),v(e),y(e)}function p(t){const e=t.gender==="Male"?"♂":"♀",i=d().includes(t.id),o=document.querySelector(".pet-detail-header");o.innerHTML=`
    <div class="pet-header-content">
      ...
      <div class="pet-header-actions">
        ...
        <button class="adopt-btn-header">Adopt Me</button>
      </div>
    </div>
  `;const r=document.querySelector(".pet-detail-story");r.innerHTML=`
    <div class="story-content">
      <h2>Story</h2>
      <p class="story-summary"><strong>${t.story.summary}</strong></p>
      <p class="story-details">${t.story.details}</p>
    </div>
  `;const c=document.querySelector(".pet-detail-traits"),s=t.traits.map(n=>`<span class="trait-tag">${n}</span>`).join("");c.innerHTML=`
    <div class="traits-content">
      <h2>Personality Traits</h2>
      <div class="traits-grid">${s}</div>
    </div>
  `;const a=document.querySelector(".pet-detail-health");if(a.innerHTML=`
  ...
  `,t.urgent&&t.urgent_reason){const n=document.querySelector(".pet-detail-urgent");n.innerHTML=`
      <div class="urgent-content">
        <h2>Why Urgent?</h2>
        <p>${t.urgent_reason}</p>
      </div>
    `}}function v(t){const e=document.querySelector(".pet-gallery-container");if(t.image_urls.length===0)return;const i=`
    <img 
  ...
  `;const r=e.querySelectorAll(".pet-gallery-thumb"),c=e.querySelector(".pet-gallery-main");r.forEach(s=>{s.addEventListener("click",()=>{const a=parseInt(s.dataset.index);c.src=t.image_urls[a],r.forEach(n=>n.classList.remove("active")),s.classList.add("active")})})}function y(t){const e=document.querySelector(".fav-btn");e&&e.addEventListener("click",i=>{i.preventDefault(),e.classList.toggle("is-liked"),e.classList.contains("is-liked")&&(e.classList.add("animate"),setTimeout(()=>{e.classList.remove("animate")},600)),u(t.id)})}document.addEventListener("DOMContentLoaded",g);

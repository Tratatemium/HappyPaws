...
function renderPetDetails(pet) {
  ...
  headerSection.innerHTML = `
   <div class="pet-header-content">
     ...
     <h1>${pet.name}</h1>
     <p class="pet-short-info">${genderIcon} · ${pet.age.value} ${
    pet.age.unit
  } · ${pet.breed}</p>
 ...
 `;
  ...
  if (favBtn.classList.contains("is-liked")) {
    favBtn.querySelector(".fav-btn-text").textContent = "Added to Favorites";
  }
  ...
  healthSection.innerHTML = `
   <div class="health-content">
     ...
     <ul class="health-list">
       <li><span class="check">✓</span> Vaccinated: ${
         pet.vaccinated ? "Yes" : "No"
       }</li>
       <li><span class="check">✓</span> Microchip: ${
         pet.chip_inserted ? "Yes" : "No"
       }</li>
       <li><span class="check">✓</span> Spayed/Neutered: ${
         pet.spayedNeutered ? "Yes" : "No"
       }</li>
       ${
         pet.specialNeeds
           ? `<li><span class="info">ℹ</span> Special Needs: ${pet.specialNeeds}</li>`
           : ""
       }
     </ul>
   </div>
...
function attachFavoriteButton(pet) {
  //its reduadant to have this line to its necessary to have it for the fav-btn to be clicked
  const favBtn = document.querySelector(".fav-btn");
  if (!favBtn) return;
  ...
  favBtn.addEventListener("click", (e) => {
    ...
    if (favBtn.classList.contains("is-liked")) {
      ...
      favBtn.querySelector(".fav-btn-text").textContent = "Add to Favorites";
    }

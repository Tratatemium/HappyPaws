...
<p>Adoption fee: 1500 SEK (includes vaccinations, microchip, and spay/neuter)*</p>
 ...
 <div class='checkbox-container'>
   ...
   <p>I accept the terms and conditions of adoption and understand the responsibilities involved in pet ownership </p>
       ...
       `,i.addEventListener("click",()=>{i.classList.toggle("active");const n=t.querySelector(".generated-terms");n?n.classList.toggle("hidden"):(t.innerHTML=e,i.classList.toggle("active"))});const o=c();if(!o){console.error("No pet ID in URL");return}const r=d(o);if(!r){console.error("Pet not found");return}u(r),t.appendChild(e)});s.addEventListener("click",e=>{let t=!0;document.querySelectorAll(".error-text").forEach(o=>{o.textContent=""}),l.forEach(o=>{const r=document.getElementById(o.id+"-error");if(o.value.trim()===""){e.preventDefault(),t=!1;const n=document.createElement("p");n.classList.add("error-text"),n.textContent=`PLease fill in your ${o.id.toUpperCase()} `,n.style.color="red",r&&r.appendChild(n)}t&&(console.log("valid"),e.preventDefault(),p("Application Submitted Successfully!"))})});function p(e){const t=document.getElementById("toast"),o=document.createElement("p");t&&(t.textContent=e,t.classList.add("show"),t.style.display="block",t.appendChild(o),setTimeout(()=>{t.classList.remove("show"),t.style.display="none"},3e3))}const m=document.querySelector(".close-button");m.addEventListener("click",()=>{window.history.length>1?history.back():window.location.href="/home"});
